...
function cleanfulltree {
   # does the clean-up op on the full source tree before building the full
   # tarball
...
# awk) is always going to be fragile, so we use a C++ preprocessor (which
...
# tarball, before building the source tarball
cleanfulltree $mode
