...
We build OpenAudacity using [Microsoft Visual Studio 2019](https://visualstudio.microsoft.com/vs/community/). In order to build OpenAudacity **Desktop development with C++** workload is required.

As we require only C++14 - MSVC 2017 should work just fine too.

### MacOS

We build OpenAudacity using XCode 12. However, it is likely possible to build it with XCode 7.
...
1. Clone OpenAudacity from the OpenAudacity GitHub project. 
  
   For example, in the **git-bash** run:

    ```
    $ git clone https://github.com/moralesresearch/OpenAudacity/
    ```

2. Open CMake GUI. 
   
   Set the **Where is the source code** to the location where OpenAudacity was cloned. 
...
7. You can now run and debug OpenAudacity!
...
2. Configure OpenAudacity using CMake:
   ```
   $ mkdir build && cd build
   $ cmake -GXcode -T buildsystem=1 ../audacity
   ```

3. Open OpenAudacity XCode project:
   ```
   $ open Audacity.xcodeproj
   ```
   and build OpenAudacity using the IDE. 

Steps 1 and 2 are only required for first-time builds. 

Alternatively, you can use **CLion**. If you chose to do so, open the directory where you have cloned OpenAudacity using CLion and you are good to go.
...
```
cmake -GXCode -T buildsystem=1 -Duse_mad="off" -Duse_id3tag=off ../openaudacity
```

## Linux & Other OS

1. Clone Audacity from the OpenAudacity GitHub project. 
...
2. Configure OpenAudacity using CMake:
   ```
   $ mkdir build && cd build
   $ cmake -G "Unix Makefiles" -Duse_ffmpeg=loaded ../openaudacity
   ```
   By default, Debug build will be configured. To change that, pass `-DCMAKE_BUILD_TYPE=Release` to CMake.

3. Build OpenAudacity:
   ```
   $ make -j`nproc`
   ```
...
On Linux it is possible to build OpenAudacity using (almost) only the libraries provided by the package manager. Please, see the list of required libraries [here](linux/required_libraries.md).
...
There is a [`Dockerfile`](linux/build-environment/Dockerfile) that can be used as an example of how to build OpenAudacity using system libraries: 
