...
$env:AWS_ACCESS_KEY_ID = $env:SIGN_KEY
...
function Upload-FilesForSigning {
    ...
    foreach ($file in $FilesToUpload) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Failed to upload ${file} for signing (exit $LASTEXITCODE)"
        }
    }
...
function Download-SignedFiles {
    ...
    for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
        ...
        foreach ($file in $FilesToDownload) {
            ...
            # Keep the extension so Windows can verify MSI signatures too.
            $fileSignedPath = "${file}.signed$([System.IO.Path]::GetExtension($file))"
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                $sig = Get-AuthenticodeSignature -LiteralPath $fileSignedPath
                if ($sig.Status -eq 'Valid') {
                    $signedFilesDownloaded += $file
                    Write-Host "File is signed. Replacing original with signed file: $file"

                    Move-Item -LiteralPath $fileSignedPath -Destination $file -Force

                    Write-Host "Removing signed file from S3: $s3SignedUrl"
                    aws s3 rm $s3SignedUrl | Out-Null
                    if ($LASTEXITCODE -ne 0) {
                        throw "Failed to remove ${s3SignedUrl} (exit $LASTEXITCODE)"
                    }
                    # Reset the counter when the queue moves
                    $attempt = 0
                } else {
                    Write-Host "Downloaded file is not properly signed: $fileSignedPath (Status: $($sig.Status))"
                    # Remove the invalid result before requesting another signature.
                    aws s3 rm $s3SignedUrl | Out-Null
                    if ($LASTEXITCODE -ne 0) {
                        throw "Failed to remove ${s3SignedUrl} (exit $LASTEXITCODE)"
                    }
                    ...
                    aws s3 cp $file $s3UnsignedUrl | Out-Null
                    if ($LASTEXITCODE -ne 0) {
                        throw "Failed to re-upload ${file} for signing (exit $LASTEXITCODE)"
                    }
                    Write-Host "Removing invalid signed file: $fileSignedPath"
                    Remove-Item -LiteralPath $fileSignedPath -Force
                    $allSigned = $false
                }
            } else {
...
if ($File -ne "") {
    ...
    $filesToProcess += (Get-Item -LiteralPath $File).FullName
    ...
    $filesToProcess = @(Get-ChildItem `
        ...
        -Recurse `
        -File | Where-Object {
            (Get-AuthenticodeSignature -LiteralPath $_.FullName).Status -ne [System.Management.Automation.SignatureStatus]::Valid
