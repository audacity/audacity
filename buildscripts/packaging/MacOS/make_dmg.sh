...
# Qt and QML dependencies were deployed by CMake during installation. Keep the
# existing rpath fixups for non-Qt libraries bundled by Audacity.
...
if [[ "$ARCH" == "x86_64" || "$ARCH" == "arm64" ]]; then
  echo "Thin universal dependencies to ${ARCH}"
  while IFS= read -r -d '' FILE_PATH; do
    ARCHS=$(lipo -archs "$FILE_PATH" 2>/dev/null) || continue
    if [[ " $ARCHS " == *" $ARCH "* && "$ARCHS" == *" "* ]]; then
      FILE_MODE=$(stat -f '%Lp' "$FILE_PATH")
      lipo "$FILE_PATH" -thin "$ARCH" -output "${FILE_PATH}.thin"
