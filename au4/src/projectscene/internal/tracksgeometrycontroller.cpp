#include "tracksgeometrycontroller.h"
...
muse::ValCh<int> TracksGeometryController::trackHeight(const processing::TrackId& trackId) const
{
...
void TracksGeometryController::changeTrackHeight(const processing::TrackId& trackId, int deltaY)
{
    auto it = m_data.find(trackId);
    if (it == m_data.end()) {
        muse::ValCh<int> val;
