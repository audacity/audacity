...
void WaveClipItem::SetInterval(std::shared_ptr<WaveTrack::Interval> interval)
{
                              ...
                              const TimelineContext& trackPanel)
{
   ...
   mClipRect = QRect(
      QPoint(
         viewRect.x() + std::max(frameLeft, -FrameRadius),
         viewRect.top()
      ),
      QPoint(
         viewRect.x() + std::min(frameLeft + frameWidth, viewRect.right() + FrameRadius),
         viewRect.bottom()
      )
   );
   ...
   auto top = viewRect.top() + HeaderHeight;
   const auto channelHeight = (viewRect.height() - HeaderHeight) / static_cast<int>(mInterval->NChannels());
