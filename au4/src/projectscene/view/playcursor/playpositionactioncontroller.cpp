...
void PlayPositionActionController::snapCurrentPosition()
{
    ...
    double newXPosition = m_context->timeToPosition(currentPlaybackPosition);
    audio::secs_t secs = m_context->positionToTime(newXPosition, true);
...
void PlayPositionActionController::applySingleStep(Direction direction)
{
    ...
    if (snapEnabled) {
        double newXPosition = m_context->timeToPosition(currentPlaybackPosition);
        secs = m_context->singleStepToTime(newXPosition, direction);
    } else {
