...
static const std::string musehubUrl = "https://www.musehub.com/plugin/";
static const std::string musehubEffectUtmSourcePrefix = "utm_source=au_main_getfx_mh-";
static const std::string musehubEffectUtmMedium = "utm_medium=mh-main-fx";
...
static std::vector<EffectsGroup> parseProductPages(const rapidjson::Document& doc) {
   if (!doc.HasMember("data") ||
       ...
       !doc["data"]["product_pages_configuration"].HasMember("museScoreStudioPageSections")) {
      assert(false);
      return {};
   }
   ...
   const rapidjson::Value& sections = doc["data"]["product_pages_configuration"]["museScoreStudioPageSections"];
...
{
   return musehubUrl + effectCode +
         "?" + musehubEffectUtmSourcePrefix + effectCode +
         "&" + musehubEffectUtmMedium;
}
