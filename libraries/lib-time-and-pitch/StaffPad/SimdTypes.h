...
/// create a c++ class at an memory-aligned spot that needs to be deleted using
/// aligned_delete
template <typename cls> inline cls* aligned_new(int alignment)
{
...
template <typename cls> inline void aligned_delete(cls* obj)
{
   if (obj != nullptr)
   {
      obj->~cls();
...
template <typename T> inline bool is_aligned(T* obj, int alignment)
{
   return (((size_t)obj) & (alignment - 1)) == 0;
}

/** this template allows to write float SIMD code with the supported operators
the following way:
...
__finl void
perform_parallel_simd_aligned(float* a, float* b, int n, const fnc& f)
{
