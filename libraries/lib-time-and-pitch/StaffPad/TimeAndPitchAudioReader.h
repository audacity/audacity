...
public:
  TimeAndPitchAudioReader(std::unique_ptr<BaseAudioReader> reader)
      : _base_reader(std::move(reader)), _sig(2, _block_size)
  {
    ...
    _silent_samples_to_append = _looping ? 0 : _time_and_pitch->getLatencySamples();
  }
  ...
  bool setPosition(int64_t newSamplePosition) override
  {
    ...
    {
      _base_reader->setPosition(newSamplePosition); // todo: needs to move further into the past to make up latency
      _initial_samples_to_discard = _time_and_pitch->getLatencySamples();
      _silent_samples_to_append = _looping ? 0 : _time_and_pitch->getLatencySamples();
  ...
  int32_t readTimeAndPitch(Signal32& outputSignal, double timeStretch = 1.0, double pitchFactor = 1.0)
  {
    ...
    while (num_samples > 0)
    {
      ...
      while (_time_and_pitch->getNumAvailableOutputSamples() < num_samples_to_retrieve)
      {
        int feed_now = std::min(_time_and_pitch->getSamplesToNextHop(), _block_size);
      ...
      if (int n = int(_initial_samples_to_discard * timeStretch + 0.5f); n > 0)
      {
        n = std::min(
            {n, _time_and_pitch->getNumAvailableOutputSamples(), outputSignal.getNumOfDataPoints(), _block_size});
        // dump to the beginning of the block to be overwritten be real data later
        float* dmp[2] = {outputSignal.getWriteChannelPtr(0), outputSignal.getWriteChannelPtr(1)};
        _time_and_pitch->retrieveAudio(dmp, n);
        _initial_samples_to_discard -= int(n / timeStretch + 0.5f);
      }
      else
      {
        _initial_samples_to_discard = 0; // can still be 1 or 2 for extreme factors due to rounding
        float* smp[2] = {outputSignal.getWriteChannelPtr(0) + p, outputSignal.getWriteChannelPtr(1) + p};

        int num_samples_retrieved = std::min(num_samples_to_retrieve, _time_and_pitch->getNumAvailableOutputSamples());
...
private:
  ...
  int32_t _read_from_base_reader_append_silence(Signal32& sig)
  {
    ...
    {
      int silent_n = std::min(sig.getNumOfDataPoints() - n, _silent_samples_to_append);
