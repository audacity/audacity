...
public:
   ...
   void readBlockWithGain(
      int startOffset, int n, SampleT* targetBlock, float gainFactor) const
   {
      _splitBlockOffsetFunction(
         startOffset, n, [=](int bufferOff, int sampleOff, int n) {
            vo::constantMultiply(
               &_buffer[bufferOff], gainFactor, &targetBlock[sampleOff], n);
         });
   }

   void readAddBlockWithGain(
      int startOffset, int n, SampleT* targetBlock, float gainFactor) const
   {
      _splitBlockOffsetFunction(
         startOffset, n, [=](int bufferOff, int sampleOff, int n) {
            vo::constantMultiplyAndAdd(
               &_buffer[bufferOff], gainFactor, &targetBlock[sampleOff], n);
         });
   }

   void writeAddBlockWithGain(
      int startOffset, int n, const SampleT* sourceBlock, float gainFactor)
   {
      _splitBlockOffsetFunction(
         startOffset, n, [=](int bufferOff, int sampleOff, int n) {
            vo::constantMultiplyAndAdd(
               &sourceBlock[sampleOff], gainFactor, &_buffer[bufferOff], n);
         });
   }
