...
WaveDataCache::DataProvider MakeDefaultDataProvider(const WaveClip& clip)
{
   return [sequence = clip.GetSequence(0), clip = &clip, appendBufferHelper = AppendBufferHelper()](
...
WaveDataCache::WaveDataCache(const WaveClip& waveClip)
   ...
   , mProvider(MakeDefaultDataProvider(waveClip))
{
}

WaveDataCache::WaveDataCache(DataProvider provider, ElementFactory elementFactory, double sampleRate)
    : GraphicsDataCache<WaveCacheElement>(sampleRate, std::move(elementFactory))
