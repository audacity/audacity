...
struct FFMPEG_SUPPORT_API AVCodecFunctions
{
    ...
    int (*avcodec_fill_audio_frame)(AVFrame* frame, int nb_channels, AVSampleFormatFwd sample_fmt, const uint8_t* buf, int buf_size,
                                    int align) = nullptr;
    ...
    int (*avcodec_get_supported_config)(const AVCodecContext* avctx, const AVCodec* codec,
                                        AVCodecConfigFwd config, unsigned flags, const void** out_configs,
                                        int* out_num_configs) = nullptr;
    ...
    //! Needed to discard decoder state after a seek. Tolerated when absent:
    //! the video backend rebuilds the codec context instead.
