...
void UpdateManager::GetUpdates(bool ignoreNetworkErrors, bool configurableNotification)
{
    ...
    response->setRequestFinishedCallback([response, ignoreNetworkErrors, configurableNotification,
                                          this](audacity::network_manager::IResponse*) {
...
#endif
        {
            gAudioIO->CallAfterRecording([this, ignoreNetworkErrors, configurableNotification] {
                ...
                if (code == wxID_YES) {
                    ...
                    downloadResponse->setRequestFinishedCallback([downloadResponse, ignoreNetworkErrors,
                                                                  this](audacity::network_manager::IResponse*) {
                        ...
                        if ((info.GetOperatingSystemId() & wxOS_WINDOWS)
                            ...
                            if (wxFileName(mAudacityInstallerPath).Exists()) {
                                std::string cmd = info.GetOperatingSystemId() & wxOS_MAC ? "Open "
                                                  + mAudacityInstallerPath : mAudacityInstallerPath;
