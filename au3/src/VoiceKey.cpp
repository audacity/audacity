        ...
        if (samplesleft > 10) {
            ...
            for (i = 0; i + WindowSizeInt < SignalWindowSizeInt; i++) {
                ...
                if (mUseSignChangesLow) {
                    TestSignChangesUpdate(sc, WindowSizeInt, buffer[i], buffer[i + 1], buffer[i + WindowSizeInt],
                                          buffer[i + WindowSizeInt + 1]);
                ...
                if (mUseDirectionChangesLow) {
                    TestDirectionChangesUpdate(dc, WindowSizeInt, atrend, buffer[i], buffer[i + 1], ztrend, buffer[i + WindowSizeInt],
                                               buffer[i + WindowSizeInt + 1]);
                ...
                if (tests >= testThreshold) { //Finish off on the first hit
                    break;
                }
            }
    ...
    if ((mWindowSize) >= (len + 10).as_double()) {
        ...
        if (samplesleft > 10) {
            ...
            for (i = 0; i < SilentWindowSizeInt - WindowSizeInt; i++) {
                ...
                if (tests < testThreshold) { //Finish off on the first below-threshold block
                    break;
                }
            }
