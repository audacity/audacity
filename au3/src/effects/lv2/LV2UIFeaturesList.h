...
struct LV2UIFeaturesList final : ExtendedLV2FeaturesList {
    ...
    LV2UIFeaturesList(
        const LV2WrapperFeaturesList& baseFeatures, UIHandler* pHandler, const LilvNode* node, LilvInstance* pInstance = nullptr,
...
public:
    ...
    static void suil_port_write(SuilController controller, uint32_t port_index, uint32_t buffer_size, uint32_t protocol,
                                const void* buffer);
    //! @}
