...
int LabelStruct::GetColorIndex() const
{
    return mColorIndex;
}

void LabelStruct::SetColorIndex(int colorIndex)
{
    ...
    , mClipLen{0.0}
{
    for (auto& original: orig.mLabels) {
        ...
        l.SetColorIndex(original.GetColorIndex());
        mLabels.push_back(l);
    }
}
...
bool LabelTrack::HandleXMLTag(const std::string_view& tag, const AttributesList& attrs)
{
    if (tag == "label") {
        ...
        l.SetColorIndex(static_cast<int>(colorIndex));
        mLabels.push_back(l);
...
// may throw
{
    ...
    for (auto& labelStruct: mLabels) {
        ...
        xmlFile.WriteAttr(wxT("colorindex"), labelStruct.GetColorIndex());
...
Track::Holder LabelTrack::Copy(double t0, double t1, bool) const
{
    ...
    for (auto& labelStruct: mLabels) {
        ...
        if (relation == LabelStruct::SURROUNDS_LABEL) {
            ...
            l.SetColorIndex(labelStruct.GetColorIndex());
    ...
    for (unsigned int i = 0; i < mLabels.size(); ++i) {
        ...
        if (relation == LabelStruct::SURROUNDS_LABEL) {
            ...
            for (int j = 1; j <= n; j++) {
                ...
                l.SetColorIndex(label.GetColorIndex());
