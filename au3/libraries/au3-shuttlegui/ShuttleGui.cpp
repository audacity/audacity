    ...
    const BoolSetting& Setting)
{
    ...
    WrappedType WrappedRef(Value);
    if (DoStep(1)) {
        DoDataShuttle(Setting.GetPath(), WrappedRef);
    }
    if (DoStep(2)) {
        pCheck = DoTieCheckBox(Prompt, WrappedRef);
    }
    if (DoStep(3)) {
        DoDataShuttle(Setting.GetPath(), WrappedRef);
    }
    ...
    const BoolSetting& Setting)
{
    ...
    if (DoStep(2)) {
        pCheck = DoTieCheckBoxOnRight(Prompt, WrappedRef);
    }
    if (DoStep(3)) {
        DoDataShuttle(Setting.GetPath(), WrappedRef);
    }
    ...
    const int min)
{
    ...
    if (DoStep(2)) {
        pSlider = DoTieSlider(Prompt, WrappedRef, max, min);
    }
    if (DoStep(3)) {
        DoDataShuttle(Setting.GetPath(), WrappedRef);
    }
    ...
    const int min)
{
    ...
    if (DoStep(2)) {
        pSpinCtrl = DoTieSpinCtrl(Prompt, WrappedRef, max, min);
    }
    if (DoStep(3)) {
        DoDataShuttle(Setting.GetPath(), WrappedRef);
    }
    ...
    const int nChars)
{
    ...
    if (DoStep(2)) {
        pText = DoTieTextBox(Prompt, WrappedRef, nChars);
    }
    if (DoStep(3)) {
        DoDataShuttle(Setting.GetPath(), WrappedRef);
    }
    ...
    const int nChars)
{
    ...
    if (DoStep(2)) {
        pText = DoTieNumericTextBox(Prompt, WrappedRef, nChars);
    }
    if (DoStep(3)) {
        DoDataShuttle(Setting.GetPath(), WrappedRef);
    }
    ...
    const int nChars, bool acceptEnter)
{
    ...
    if (DoStep(2)) {
        pText = DoTieNumericTextBox(Prompt, WrappedRef, nChars, acceptEnter);
    }
    if (DoStep(3)) {
        DoDataShuttle(Setting.GetPath(), WrappedRef);
    }
...
//   int TempIndex = TranslateToIndex( Default, InternalChoices );
    ...
    if (DoStep(1)) {
        DoDataShuttle(SettingName, WrappedRef);              // Get Index from Prefs.
    }
    if (DoStep(1)) {
        TempIndex = TranslateToIndex(TempStr, InternalChoices);              // To an index
    }
    if (DoStep(2)) {
        pChoice = TieChoice(Prompt, TempIndex, Choices);
    }
    if (DoStep(3)) {
        TempStr = TranslateFromIndex(TempIndex, InternalChoices);              // To a string
    }
    if (DoStep(3)) {
        choiceSetting.Write(TempStr);            // Put into Prefs.
    }
    return pChoice;
}
