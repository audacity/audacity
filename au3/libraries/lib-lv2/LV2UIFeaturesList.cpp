...
LV2UIFeaturesList::LV2UIFeaturesList(
    ...
    const LilvNode* node, LilvInstance* pInstance, void* windowHandle)
    : ExtendedLV2FeaturesList{WithBase, baseFeatures}
    , mpHandler{pHandler}
    , mOk{InitializeFeatures(node, pInstance, windowHandle)}
{
}

bool LV2UIFeaturesList::InitializeFeatures(const LilvNode* node,
                                           LilvInstance* pInstance, void* windowHandle)
{
    ...
    AddFeature(LV2_UI__parent, windowHandle);
...
{
    static_cast<UIHandler*>(handle)->ui_resize(width, height);
}
