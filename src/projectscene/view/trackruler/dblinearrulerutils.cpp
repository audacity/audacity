...
double valueToPosition(double value, double height, bool isNegativeSample)
{
    const auto linearValue = muse::db_to_linear(value) * (isNegativeSample ? -1.0 : 1.0);
    return (0.5 - linearValue / 2.0) * height;
}
...
{
    const int lowestFullStep = computeLowestFullStepValue(height, dbRange, minHeightToZero);
