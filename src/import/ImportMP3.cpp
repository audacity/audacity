                                           ...
                                           Tags *tags)
{
   ...
   // And save any ID3 tags loaded from the file
   LoadID3(tags);
...
bool MP3ImportFileHandle::Open()
{
   ...
   // Check for and load any ID3 tags
   if (!CheckID3())
   {
      return false;
   }
...
bool MP3ImportFileHandle::CheckID3()
{
   ...
   if (memcmp(mInputBuffer, "3DI", 3) == 0)
   {
      ...
      // Yes, so tags from the file's length
      // It's 10 for the 3DI header, len for the tag data, and 10 more for the ID3 header
      mFileLen -= (10 + len + 10);
