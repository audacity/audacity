   ...
   const SpectrogramSettings& settings) const
{
   ...
   // a sample period.
   const bool ppsMatch = (fabs(samplesPerPixel - spp) * len < 1.0);
   ...
   float* __restrict out) const
{
   ...
   const auto offset = clip.GetSequenceStartTime();
   ...
   if (from < 0 || from >= numSamples) {
...

      {
         ...
         if (myLen > 0) {
            ...
            mSampleCacheHolder.emplace(clip.GetSampleView(iChannel, from, myLen));
            floats.resize(myLen);
            FillBufferFromTrackBlockSequence(
               { *mSampleCacheHolder }, floats.data(), myLen);
   ...
   double pixelsPerSecond)
{
   ...
   mSpecCache->Grow(
      numPixels, settings, samplesPerPixel, t0 /*Grow takes absolute t0*/);
