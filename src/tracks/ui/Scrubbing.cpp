...
Scrubber &Scrubber::Get( SneedacityProject &project )
{
   return project.AttachedObjects::Get< Scrubber >( key );
}

const Scrubber &Scrubber::Get( const SneedacityProject &project )
{
   return Get( const_cast< SneedacityProject & >( project ) );
}

Scrubber::Scrubber(SneedacityProject *project)
   : mScrubToken(-1)
   , mPaused(true)
      ...
      []{
         ...
         for (const auto &item : menuItems()) {
            ptrs.push_back( Command( item.name, item.label,
               ...
               item.StatusTest
                  ? // a checkmark item
                     Options{}.CheckTest( [&item](SneedacityProject &project){
