 ...
 NotifyingSelectedRegion &newSel)
{
   ...
   if(evt.Dragging())
   {
      ...
      auto naviagationIndex = view.GetNavigationIndex(project);
      if(naviagationIndex != -1 && 
          (naviagationIndex == hit.mMouseOverLabel ||
              naviagationIndex == hit.mMouseOverLabelLeft ||
              naviagationIndex == hit.mMouseOverLabelRight))
      {
         //Set the selection region to be equal to
         //the NEW size of the label.
         newSel = mLabels[naviagationIndex].selectedRegion;
      }
      pTrack->SortLabels();
   }
