...
void DoSelectClipBoundary(SneedacityProject &project, bool next)
{
...
(SneedacityProject &project, const WaveTrack* wt, double t0, double t1)
{
...
void DoSelectClip(SneedacityProject &project, bool next)
{
   auto &selectedRegion = ViewInfo::Get( project ).selectedRegion;
   auto &trackFocus = TrackFocus::Get( project );
...
(SneedacityProject &project, bool next)
{
   auto &selectedRegion = ViewInfo::Get( project ).selectedRegion;
   auto &trackFocus = TrackFocus::Get( project );
   auto &window = ProjectWindow::Get( project );
...
double DoClipMove( SneedacityProject &project, Track *track,
...
(SneedacityProject &project, bool right, bool keyUp )
{
   ...
   if (amount != 0.0) {
      ...
      // entry in Sneedacity's history dialog.
