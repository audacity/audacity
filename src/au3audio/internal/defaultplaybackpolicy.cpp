...
void DefaultPlaybackPolicy::Initialize(PlaybackSchedule& schedule, double rate)
{
    PlaybackPolicy::Initialize(schedule, rate);
    mLastPlaySpeed = GetPlaySpeed();
    // mMessageChannel.Initialize()
    mMessageChannel.Write({mLastPlaySpeed, schedule.mT0, mLoopEndTime, mLoopEnabled});

    auto callback = [this](auto&) {
        WriteMessage();
    };
    mRegionSubscription = ViewInfo::Get(mProject).playRegion.Subscribe(callback);
    if (mVariableSpeed)
...
PlaybackPolicy::BufferTimes DefaultPlaybackPolicy::SuggestedBufferTimes(PlaybackSchedule&)
{
...
bool DefaultPlaybackPolicy::Done(PlaybackSchedule& schedule, unsigned long outputFrames)
{
...
double DefaultPlaybackPolicy::OffsetSequenceTime(PlaybackSchedule& schedule, double offset)
{
...
PlaybackSlice DefaultPlaybackPolicy::GetPlaybackSlice(PlaybackSchedule& schedule, size_t available)
{
    ...
    if (deltat > realTimeRemaining) {
        ...
        if (RevertToOldDefault(schedule)) {
            ...
            const double extraRealTime = ((TimeQueueGrainSize + 1) / mRate) * mLastPlaySpeed;
...
std::pair<double, double> DefaultPlaybackPolicy::AdvancedTrackTime(PlaybackSchedule& schedule, double trackTime, size_t nSamples)
{
...
bool DefaultPlaybackPolicy::RepositionPlayback(PlaybackSchedule& schedule, const Mixers& playbackMixers, size_t frames, size_t available)
{
    ...
    mLoopEnabled = data.mLoopEnabled && !empty && schedule.mTimeQueue.GetLastTime() <= data.mT1 + allowance;
    ...
    if ((loopWasEnabled != mLoopEnabled) || (mine != theirs)) {
      ...
      newTime = std::clamp(newTime, schedule.mT0, schedule.mT1);
        ...
        if (RevertToOldDefault(schedule))
            return PlaybackPolicy::RepositionPlayback(schedule, playbackMixers, frames, available);
    }
    ...
    if (mRemaining <= 0) {
        ...
        for (auto& pMixer : playbackMixers)
            pMixer->SetTimesAndSpeed(schedule.mT0, schedule.mT1, mLastPlaySpeed, true);
...
void DefaultPlaybackPolicy::WriteMessage()
{
    ...
    mMessageChannel.Write({GetPlaySpeed(), region.GetStart(), region.GetEnd(), region.Active()});
}

double DefaultPlaybackPolicy::GetPlaySpeed()
{
    return mVariableSpeed ? ProjectAudioIO::Get(mProject).GetPlaySpeed() : 1.0;
}
