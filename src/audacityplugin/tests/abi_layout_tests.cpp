...
#define AUP_ASSERT_SIZE(type_, size_) \
    static_assert(sizeof(type_) == (size_), #type_ " has an unexpected size")
#define AUP_ASSERT_ALIGNMENT(type_, alignment_) \
    static_assert(alignof(type_) == (alignment_), \
                  #type_ " has an unexpected alignment")
#define AUP_ASSERT_OFFSET(type_, field_, offset_) \
    static_assert(offsetof(type_, field_) == (offset_), \
                  #type_ "." #field_ " has an unexpected offset")

static_assert(sizeof(void*) == 8, "Plugin API v0 supports native 64-bit hosts only");
static_assert(AUP_API_MAJOR == 0U && AUP_API_REVISION == 1U,
