...
\class SneedacityLogger
\brief SneedacityLogger is a thread-safe logger class
...
SneedacityLogger *SneedacityLogger::Get()
{
   ...
   std::call_once( flag, []{
      ...
      std::unique_ptr < wxLog > // DELETE any previous logger
         { wxLog::SetActiveTarget(safenew SneedacityLogger) };
   } );

   // Use dynamic_cast so that we get a NULL ptr in case our logger
   // is no longer the target.
   return dynamic_cast<SneedacityLogger *>(wxLog::GetActiveTarget());
}

SneedacityLogger::SneedacityLogger()
:  wxEvtHandler(),
   wxLog()
{
   mText = NULL;
...
SneedacityLogger::~SneedacityLogger()  = default;

void SneedacityLogger::Flush()
{
...
void SneedacityLogger::DoLogText(const wxString & str)
{
   if (!wxIsMainThread()) {
      wxMutexGuiEnter();
   }
   ...
   if (mBuffer.empty()) {
      ...
      mBuffer << stamp << _TS("Sneedacity ") << SNEEDACITY_VERSION_STRING << wxT("\n");
   }
...
bool SneedacityLogger::SaveLog(const wxString &fileName) const
{
...
bool SneedacityLogger::ClearLog()
{
...
void SneedacityLogger::Show(bool show)
{
   ...
   frame->Bind(wxEVT_CLOSE_WINDOW,
                  wxCloseEventHandler(SneedacityLogger::OnCloseWindow),
                  this);

   frame->Bind(   wxEVT_COMMAND_MENU_SELECTED,
                  &SneedacityLogger::OnSave,
                  this, LoggerID_Save);
   frame->Bind(   wxEVT_COMMAND_MENU_SELECTED,
                  &SneedacityLogger::OnClear,
                  this, LoggerID_Clear);
   frame->Bind(   wxEVT_COMMAND_MENU_SELECTED,
                  &SneedacityLogger::OnClose,
                  this, LoggerID_Close);
...
wxString SneedacityLogger::GetLog(int count)
{
   if (count == 0)
   {
      return mBuffer;
...
void SneedacityLogger::OnCloseWindow(wxCloseEvent & WXUNUSED(e))
{
...
void SneedacityLogger::OnClose(wxCommandEvent & WXUNUSED(e))
{
   wxCloseEvent dummy;
   OnCloseWindow(dummy);
}

void SneedacityLogger::OnClear(wxCommandEvent & WXUNUSED(e))
{
   ClearLog();
}

void SneedacityLogger::OnSave(wxCommandEvent & WXUNUSED(e))
{
...
void SneedacityLogger::UpdatePrefs()
{
   if (mFrame) {
