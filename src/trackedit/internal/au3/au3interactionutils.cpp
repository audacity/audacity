...
au::trackedit::ClipConversionType addClipToTrack(const std::shared_ptr<au::au3::Au3WaveClip>& clip,
                                                 ...
                                                 au::au3::Au3TrackList& copy)
{
    ...
    const auto empties = getEmptyWaveTrackIndices(copy);
    if (muse::contains(empties, dstTrackIndex) && dstWaveTrack.NChannels() != clip->NChannels()) {
    ...
    if (clip->NChannels() == 2 && ptr->NChannels() == 1) {
        return au::trackedit::ClipConversionType::StereoToMono;
    } else if (clip->NChannels() == 1 && ptr->NChannels() == 2) {
        return au::trackedit::ClipConversionType::MonoToStereo;
    } else {
        return au::trackedit::ClipConversionType::NoConversion;
    }
}
}
...
void au::trackedit::utils::exchangeContent(au3::Au3TrackList& tracks, au3::Au3WaveTrack& oldOne, au3::Au3WaveTrack& newOne)
{
...
au::au3::Au3WaveTrack* au::trackedit::utils::toggleStereo(au3::Au3TrackList& tracks, au3::Au3WaveTrack& track)
{
    ...
    exchangeContent(tracks, track, *replacement);
...
au::trackedit::ClipConversionType au::trackedit::utils::moveClipsUpOrDown(int offset, const au3::Au3TrackList& orig,
                                                                          ...
                                                                          const trackedit::ClipKeyList& selectedClips)
{
    ...
    trackedit::ClipConversionType requiredConversion = trackedit::ClipConversionType::NoConversion;
    ...
    for (const auto& clip : movedClips) {
        ...
        requiredConversion |= addClipToTrack(clip.ptr, *dstTrack, copy);
    }
...
au::trackedit::TrackListInfo au::trackedit::utils::getTrackListInfo(const au3::Au3TrackList& tracks)
{
    ...
    return { tracks.Size(), std::move(stereos), std::move(empties) };
}
