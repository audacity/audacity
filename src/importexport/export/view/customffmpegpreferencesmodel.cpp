...
QString CustomFFmpegPreferencesModel::ffmpegFormat() const
{
    ...
    if (format.isEmpty()
        ...
        format = ffmpegFormatList().front().toString();
    }
...
void CustomFFmpegPreferencesModel::showAllFormats()
{
    ffmpegOptionsAccessor()->showAllFormats();
...
int CustomFFmpegPreferencesModel::ffmpegCodecIndex() const
{
    ...
    for (int i = 0; i < static_cast<int>(codecs.size()); ++i) {
...
QString CustomFFmpegPreferencesModel::ffmpegCodec() const
{
    ...
    if (codec.isEmpty()
        ...
        codec = ffmpegFormatList().front().toString();
    }
...
void CustomFFmpegPreferencesModel::showAllCodecs()
{
    ffmpegOptionsAccessor()->showAllCodecs();
    emit ffmpegCodecListChanged();
    emit ffmpegCodecIndexChanged();
}
...
void CustomFFmpegPreferencesModel::setPdOMethod(const QString& pdOMethod)
{
    int idx = 0;
    for (const auto& method: ffmpegOptionsAccessor()->predictionOrderMethodList()) {
        if (QString::fromStdString(method) == pdOMethod) {
            break;
        }
        idx++;
    }

    exportConfiguration()->setFFmpegPredictionOrderMethod(idx);
}
