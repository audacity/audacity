...
namespace {
static QVariant toQVariant(const ExportValue& v)
{
...
static ExportValue fromQVariant(const QVariant& v)
{
...
static bool valuesEqual(const au::importexport::ExportValue& a, const au::importexport::ExportValue& b)
{
    if (a.index() != b.index()) {
        return false;
    }
...
bool DynamicExportOptionsModel::setData(const QModelIndex& idx, const QVariant& v, int role)
{
    ...
    std::optional<au::importexport::ExportValue> value_opt = exporter()->value(option.id);
    if (!value_opt.has_value()) {
        return false;
    }
    au::importexport::ExportValue value = value_opt.value();
