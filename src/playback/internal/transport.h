...
//! Coordinates the player and the recorder. Holds the playback session state and
//! the transport state machine; delegates the actual stream operations to IPlayer
//! and reads recording state from IRecordController. Because both collaborators
//! are interfaces, this class is unit-testable with mocks (no au3 globals).
...
private:
    ...
    const IPlayerPtr& player() const { return m_player; }
