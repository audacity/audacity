...
inline muse::Ret make_ret(Err e)
{
    ...
    case Err::UnknownError:
        return muse::Ret(retCode);
    case Err::RecordingError:
        return muse::Ret(retCode, muse::trc("record", "Error opening recording device.\nError code: %1"));
    case Err::RecordingStopError:
        return muse::Ret(retCode, muse::trc("record", "Cannot stop recording"));
    case Err::MismatchedSamplingRatesError:
        return muse::Ret(retCode, muse::trc("record", "The tracks selected for recording must all have the same sampling rate"));
    case Err::TooFewCompatibleTracksSelected:
        return muse::Ret(
            retCode,
            muse::trc(
                "record",
