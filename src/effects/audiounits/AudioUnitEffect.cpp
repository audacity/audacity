...
struct AudioUnitEffect::Instance
   ...
   , AudioUnitWrapper
{
   Instance(AudioUnitEffect &effect, const AudioUnitEffectSettings &settings)
...
AudioUnitEffect::DoMakeInstance(EffectSettings &settings)
{
   ...
   return std::make_shared<Instance>(*this, GetSettings(settings));
}
...
bool AudioUnitEffect::InitializePlugin()
{
   ...
   // Query the instance for parameters and their values, and save them in
   ...
   const wxCFStringRef &cfname, bool binary) const
{
   ...
   // an AudioUnit object (mUnit.get()) to accomplish its work.
