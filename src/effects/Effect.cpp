...
\class Effect
\brief Base class for many of the effects in Sneedacity.
...
EffectFamilySymbol Effect::GetFamily()
{
   ...
   // effect family.
   return { wxT("Sneedacity"), XO("Built-in") };
}
                      ...
                      const EffectDialogFactory &dialogFactory)
{
   ...
   auto &pProject = *const_cast<SneedacityProject*>(FindProject()); // how to remove this const_cast?
...
bool Effect::Process()
{
   ...
   // the parameters (the Sneedacity Reverb effect does when the stereo width is 0).
   mNumAudioIn = GetAudioInCount();
   mNumAudioOut = GetAudioOutCount();
                          ...
                          ArrayOf< float *> &outBufPos)
{
   ...
   while (inputRemaining != 0 || delayRemaining != 0)
   {
      ...
      catch( const SneedacityException & WXUNUSED(e) )
      {
         // PRL: Bug 437:
...
void Effect::CopyInputTracks(bool allSyncLockSelected)
{
   ...
   mOutputTracks = TrackList::Create(
      const_cast<SneedacityProject*>( FindProject() ) // how to remove this const_cast?
  );
...
const SneedacityProject *Effect::FindProject() const
{
   if (!inputTracks())
      return nullptr;
