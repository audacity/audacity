...
ReverbEffect::Instance::Instance(const PerTrackEffect& effect) : PerTrackEffect::Instance{effect}
{
}

bool ReverbEffect::Instance::ProcessInitialize(EffectSettings& settings, double sampleRate, ChannelNames chanMap)
{
    ...
    return InstanceInit(settings, sampleRate, mState, chanMap, /* forceStereo = */ false);
}
...
)
{
    ...
    bool isStereo = false;
    state.mNumChans = 1;
    if ((chanMap && chanMap[0] != ChannelNameEOL && chanMap[1] == ChannelNameFrontRight) || forceStereo) {
        isStereo = true;
        state.mNumChans = 2;
    }
...
bool ReverbEffect::Instance::RealtimeInitialize(EffectSettings& settings, double sampleRate)
{
    SetBlockSize(512);
    mSlaves.clear();
...
bool ReverbEffect::Instance::RealtimeAddProcessor(EffectSettings& settings, EffectOutputs*, unsigned numChannels, float sampleRate)
{
...
size_t ReverbEffect::Instance::ProcessBlock(EffectSettings& settings, const float* const* inBlock, float* const* outBlock, size_t blockLen)
{
    ...
    while (remaining) {
        ...
        if (state.mNumChans == 2) {
            for (decltype(len) i = 0; i < len; i++) {
                for (int w = 0; w < 2; w++) {
                    ochans[w][i] = dryMult * state.mP[w].dry[i] + 0.5 * (state.mP[0].wet[w][i] + state.mP[1].wet[w][i]);
                }
            }
        } else {
            for (decltype(len) i = 0; i < len; i++) {
...
OptionalMessage ReverbEffect::LoadFactoryPreset(int id, EffectSettings& settings) const
{
...
bool au::effects::operator==(const ReverbSettings& a, const ReverbSettings& b)
{
    ...
    return muse::is_equal(a.mRoomSize, b.mRoomSize) && muse::is_equal(a.mPreDelay, b.mPreDelay) &&
           muse::is_equal(a.mReverberance, b.mReverberance) && muse::is_equal(a.mHfDamping, b.mHfDamping) &&
           muse::is_equal(a.mToneLow, b.mToneLow) && muse::is_equal(a.mToneHigh, b.mToneHigh) && muse::is_equal(a.mWetGain, b.mWetGain) &&
           muse::is_equal(a.mDryGain, b.mDryGain) && muse::is_equal(a.mStereoWidth, b.mStereoWidth) && (a.mWetOnly == b.mWetOnly);
}

bool au::effects::OnlySimpleParametersChanged(const ReverbSettings& a, const ReverbSettings& b)
{
    ...
    const bool oneSimpleParameterChanged =
        ...
        !muse::is_equal(a.mReverberance, b.mReverberance) || !muse::is_equal(a.mHfDamping, b.mHfDamping) ||
        !muse::is_equal(a.mToneLow, b.mToneLow) || !muse::is_equal(a.mToneHigh, b.mToneHigh) || (a.mWetGain != b.mWetGain);

    const bool allNonSimpleParametersStayedTheSame =

        muse::is_equal(a.mRoomSize, b.mRoomSize) && muse::is_equal(a.mPreDelay, b.mPreDelay) &&
