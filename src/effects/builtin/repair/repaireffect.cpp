    ...
    for (auto track : outputs.Get().Selected<WaveTrack>()) {
        ...
        if (repair_deltat > 0) {  // selection is within track audio
            ...
            if (TimeStretching::HasPitchOrSpeed(*track, repair_t0, repair_t1)) {
                mLastError = XO("The Repair effect cannot be applied within stretched or shrunk clips").Translation();
                bGoodResult = false;
                break;
            }
            if (repairLen > 128) {
                mLastError = XO("The Repair effect is intended to be used on very short sections of damaged audio (up to 128 "
            ...
            if (s0 == repair0 && s1 == repair1) {
                mLastError = XO("The Repair effect needs some data to go on.\n\nPlease select an area to repair with some audio on at "
                                "least one side (the more the better).")
                                 .Translation();
                bGoodResult = false;
                break;
            }
...
bool Repair::ProcessOne(int count, WaveChannel& track, sampleCount start, size_t len, size_t repairStart, size_t repairLen)
{
    Floats buffer{len};
