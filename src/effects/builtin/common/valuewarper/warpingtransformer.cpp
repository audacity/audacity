 ...
 * Now we let y(1/2) = mid, i.e., the value when in the middle, and solve the equation for C:
 * C = 2 * log((1 - u) / u)
 */
double solveC(double min, double mid, double max)
{
    const double u = Normalized{ mid, min, max }.value;
    const auto C = 2 * std::log((1 - u) / u);
    return C;
}
}

WarpingTransformer::WarpingTransformer(double min, double mid, double max)
    : m_C{solveC(min, mid, max)}
    , m_expC{std::exp(m_C)},
    m_min{min},
    m_max{max}
{}
