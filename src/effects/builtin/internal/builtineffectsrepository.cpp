...
void BuiltinEffectsRepository::updateEffectMetaList()
{
    ...
    for (const PluginDescriptor& desc : PluginManager::Get().PluginsOfType(PluginTypeEffect)) {
        ...
        if (symbol == AmplifyEffect::Symbol) {
            ...
            regMeta(desc, muse::mtrc("effects", "Fade In"), muse::mtrc("effects", "Applies a linear fade-in to the selected audio"), true);
            ...
            regMeta(desc, muse::mtrc("effects", "Repair"), muse::mtrc("effects", "Sets the peak amplitude of a one or more tracks"), false);
        } else if (symbol == ReverseEffect::Symbol) {
            regMeta(desc, muse::mtrc("effects", "Reverse"), muse::mtrc("effects", "Reverses the selected audio"), true);
            ...
            regMeta(desc, muse::mtrc("effects", "Reverb"), muse::mtrc("effects", "Reverb effect"), true);
            ...
            regMeta(desc, muse::mtrc("effects/noise", "Noise"), muse::mtrc("effects/noise", "Generates noise"), false);
            ...
            regMeta(desc, muse::mtrc("effects/dtmf", "DTMF Tones"), muse::mtrc("effects/dtmf", "Generates DTMF signal"), false);
        } else {
...
EffectMetaList BuiltinEffectsRepository::effectMetaList() const
{
    ...
    std::transform(m_metas.begin(), m_metas.end(), list.begin(), [](const auto& pair) {
        return pair.second;
    });
    return list;
}
