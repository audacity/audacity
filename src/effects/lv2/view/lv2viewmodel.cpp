...
bool Lv2ViewModel::buildFancy()
{
    ...
    if (auto idleUi = tryCreateLv2IdleUi(*m_suilInstance, uiType == node_ExternalUI)) {
        m_pluginUi = std::move(idleUi);
    } else {
        const auto uri = lilv_node_as_uri(uiType);
        if (!strcmp(uri, LV2_UI__GtkUI)) {
            m_pluginUi = std::make_unique<Gtk2Ui>(*m_suilInstance, [this] { onUiClosed(); }, Gtk2Ui::Params { m_title, m_minimumWidth });
        } else {
...
{
    if (std::holds_alternative<ILv2IdleUiUPtr>(m_pluginUi) && !std::get<ILv2IdleUiUPtr>(m_pluginUi)->idleFloatingUi()) {
