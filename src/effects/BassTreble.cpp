...
bool BassTrebleDialog::TransferDataFromWindow()
{
   ...
   // Ensure that max values can never exceed design limits (bug 683).
   // If this assert can be triggered on any platform (Windows?) enable the
   // next three lines.
   wxASSERT((mBass <= DB_MAX) && (mTreble <= DB_MAX) && (mLevel <= 0.0));   
