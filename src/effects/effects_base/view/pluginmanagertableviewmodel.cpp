...
QString effectDisplayName(const EffectMeta& meta)
{
    ...
    // Never-validated entries carry no metadata. Their id is the binary's
    // basename (Discovered placeholder / failed registration), unless it's a
    // full effect id we can parse a name out of.
...
void PluginManagerTableViewModel::componentComplete()
{
    ...
    m_initialState.erase(std::remove_if(m_initialState.begin(), m_initialState.end(), [](const EffectMeta& meta) {
        ...
        // Only validated entries carry a trustworthy type: non-validated ones have sparse
        // meta (type Unknown), and must stay visible so the Status column can report them.
...
void PluginManagerTableViewModel::setStatusSelectedIndex(int index)
{
    ...
    if (index == 0) {
        ...
        m_acceptStatus = [target = statusFilterOrder[index - 1]](const EffectMeta& meta) {
            // Match what the Status column displays: Undefined shows as Broken,
            // so the Broken filter must catch it too.
