...
namespace {
void buildList(au::audio::IAudioEngine& engine, ::AudacityProject& project, ChannelGroup* group, RealtimeEffectList& list)
{
    for (auto i = 0u; i < list.GetStatesCount(); ++i) {
        ...
        engine.reloadRealtimeEffectState(project, group, *state);
    }
}
}
...
void RealtimeEffectService::buildNewlyValidatedRealtimeEffects()
{
    ...
    for (WaveTrack* track : trackList.Any<WaveTrack>()) {
        buildList(*audioEngine(), *au3Project, track, RealtimeEffectList::Get(*track));
    }
    buildList(*audioEngine(), *au3Project, RealtimeEffectManager::MasterGroup, RealtimeEffectList::Get(*au3Project));
}
