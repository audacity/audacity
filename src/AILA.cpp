   ...
   do
   {
      ...
      try
      {
         if( mNumPlaybackChannels > 0 ) {
            ...
            if (mNumPlaybackChannels > 0) {
               mScratchBuffers.resize(mNumPlaybackChannels + 1);
               mScratchPointers.clear();
            ...
            for (size_t ii = 0;
               ii < std::min(MaxPlaybackChannels, mNumPlaybackChannels); ++ii)
            ...
            for (auto &track : mPlaybackTracks) {
               ...
               for (size_t jj = 0;
                  jj < std::min(MaxPlaybackChannels, mNumPlaybackChannels);
                  ++jj)
...
static void debugTimes(std::optional<double> &oTime1, double time2, double rate)
{
   if (!oTime1) {
...
void AudioIO::PollUser(PlaybackState &state, size_t newFrames)
{
   ...
   if (!pMessage) {
      ...
      if (pMessage) {
         ...
         if (pMessage != mpNewestMessage) {
            if (mpNewestMessage) {
               // Older messages point only to newer ones, so no leaky pointer
               // cycles. In fact old ones will be destroyed before newer ones,
               // so a stupid pointer would be safe, but anyway remove the doubt
               // about that
               mpNewestMessage->mpNext = pMessage;
               // This function guarantees that the frame counts will be
               // strictly increasing along the chain of messages
               assert(mpNewestMessage->mCumulativeFrames
                  < pMessage->mCumulativeFrames);
            }
            mpNewestMessage = pMessage;
         }
         else
            // The same shared pointer to message was taken again from the
            // message buffer, which had no intervening new messages
            ;
      }
      else
         // From the post of PollUser; mpNewestMessage should never have been
         // assigned
         assert(!mpNewestMessage);
   }
}
   ...
   PlaybackState &state, size_t demand, const bool paused)
{
   if (!paused) {
      ...
      if (const auto slice =
         policy.GetPlaybackSlice(playbackSchedule, state, demand);
         slice.toProduce > 0)
         return slice;
      else
         // Policy can't make progress, so treat this as a pause
         ;
   }
   ...
   std::optional<RealtimeEffects::ProcessingScope> &pScope, size_t demand)
{
   ...
   do {
      const auto discarded =
         ProcessPlaybackSlicesPass(pScope, demand, preprocessed);
...
size_t AudioIO::ProcessPlaybackSlicesPass(
   std::optional<RealtimeEffects::ProcessingScope> &pScope,
   const size_t orig_demand, size_t preprocessed)
{
   ...
   do {
      const auto slice =
         FindSlice(mPlaybackSchedule, state, demand, (paused || done));
   ...
   const auto discardable =
      ApplyEffectStack(pScope, nullptr, mMasterBuffers.begin(), preprocessed);
   ...
   std::optional<double> &debugPrevTime, std::optional<double> &debugNextTime)
{
   ...
   for (auto &track : mPlaybackTracks) {
      ...
      do {
         ...
         const auto slice = FindSlice(
            mPlaybackSchedule, trackState, limitedDemand, (paused || done));
         const auto &[frames, toProduce] = slice;

         auto &mixer = *track.mpMixer;
         const auto buffers = track.mBuffers.begin();
         ConsumeFromMixer(frames, toProduce, mixer, vt->NChannels(), buffers);

         auto discarded = ApplyEffectStack(pScope, pGroup, buffers, 0);
         ...
         myLastTime = mPlaybackSchedule.GetPolicy().AdvancedTrackTime(
            mPlaybackSchedule, trackState, myLastTime, toProduce);

         // Get new parameters from the user, or change parameters in use for
         // this track to agree with what was done already polled for another
         // track.
         // Advance count of frames without deduction for latency -- that is how
         // states of different tracks might diverge.
         PollUser(trackState, frames);

         // When processing latency, decrease demand by less, so that the loop
         // may repeat and the state may run ahead, fetching more from the track
         // (looping as required), or feeding more silence into the effect
         // processor for pause or end of play.
         assert(discarded <= frames);
         const auto fewerFrames = frames - std::min(frames, discarded);

         if (!(paused || done))
            // Update the track state
            done = policy.RepositionPlayback(mPlaybackSchedule, trackState,
               pMessage ? *pMessage : sDefaultMessage, &mixer,
               limitedDemand - fewerFrames)
            && (demand == limitedDemand);
         demand -= fewerFrames;
      } while (demand && !done);
      if (!hasLatency)
   ...
   std::unique_ptr<RingBuffer> playbackBuffers[])
{
   ...
   if (frames > 0) {
      ...
      for (size_t j = 0; j < mNumPlaybackChannels; ++j) {
         // Replicate into two RingBuffers when playing a mono track into
         // a stereo device
         const auto warpedSamples = mixer.GetBuffer(std::min(j, nChannels - 1));
         const auto put [[maybe_unused]] = playbackBuffers[iChannel++]->Put(
   ...
   std::unique_ptr<RingBuffer> playbackBuffers[], size_t preprocessed)
{
   ...
   for (unsigned iBlock : {0, 1}) {
      ...
      for (size_t iChannel = 0;
         iChannel < std::min(mNumPlaybackChannels, MaxPlaybackChannels);
         ++iChannel)
      {
   ...
   float *outputBuffer, size_t framesPerBuffer, size_t toGet)
{
   ...
   for (size_t c = 0;
      c < std::min(MaxPlaybackChannels, mNumPlaybackChannels); ++c) {
   ...
   float *outputFloats, size_t len)
{
   if (len > 0) {
      ...
      for (size_t i = 0; i < len; ++i) {
         ...
         outputFloats[mNumPlaybackChannels * i] *= multiplier;
         outputFloats[mNumPlaybackChannels * i + 1] *= multiplier;
         diff *= factor;
      }
